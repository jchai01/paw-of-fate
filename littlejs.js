 ...
 *  @param {number}  [time]
 *  @memberof Debug */
function debugLine(posA, posB, color, thickness = 0.1, time) {
...
function debugOverlap(pA, sA, pB, sB, color) {
    const minPos = vec2(
        min(pA.x - sA.x / 2, pB.x - sB.x / 2),
        min(pA.y - sA.y / 2, pB.y - sB.y / 2),
    );
    const maxPos = vec2(
        max(pA.x + sA.x / 2, pB.x + sB.x / 2),
        max(pA.y + sA.y / 2, pB.y + sB.y / 2),
    );
...
function debugSaveText(text, filename = "text", type = "text/plain") {
    debugSaveDataURL(
        URL.createObjectURL(new Blob([text], { type: type })),
        filename,
    );
}
...
function debugShowErrors() {
    const showError = (message) => {
        ...
        document.body.innerHTML =
            `<pre style=color:#f00;font-size:50px;white-space:pre-wrap>` + message;
    };
...
function debugUpdate() {
    ...
    if (debugOverlay) {
        ...
        if (keyWasPressed("Digit5")) debugScreenshot();
    }
}
...
function debugRender() {
        ...
        for (let i = gamepads.length; i--;) {
            ...
            if (gamepad) {
                ...
                for (let j = sticks.length; j--;) {
                    const drawPos = centerPos.add(
                        vec2(j * stickScale * 2, i * stickScale * 3),
                    );
                ...
                for (let j = gamepad.buttons.length; j--;) {
                    const drawPos = centerPos.add(
                        vec2(
                            j * buttonScale * 2,
                            i * stickScale * 3 - stickScale - buttonScale,
                        ),
                    );
    ...
    if (debugOverlay) {
        ...
        for (const o of engineObjects) {
            ...
            if (o instanceof TileLayer) continue; // prevent tile layers from being picked
    ...
    if (debugObject) {
        ...
        raycastHitPos &&
            drawRect(
                raycastHitPos.floor().add(vec2(0.5)),
                vec2(1),
                rgb(0, 1, 1, 0.3),
            );
        drawLine(
            ...
            debugObject.pos,
            0.1,
            raycastHitPos ? rgb(1, 0, 0, 0.5) : rgb(0, 1, 0, 0.5),
            false,
        );
...

    {
        ...
        if (debugOverlay) {
            ...
            for (const i in inputData[0]) {
                if (keyIsDown(i, 0)) keysPressed += i + " ";
            }
            keysPressed &&
                overlayContext.fillText("Keys Down: " + keysPressed, x, (y += h));

            let buttonsPressed = "";
            if (inputData[1])
                for (const i in inputData[1]) {
                    if (keyIsDown(i, 1)) buttonsPressed += i + " ";
                }
            buttonsPressed &&
                overlayContext.fillText("Gamepad: " + buttonsPressed, x, (y += h));
        } else {
            overlayContext.fillText(debugPhysics ? "Debug Physics" : "", x, (y += h));
            overlayContext.fillText(
                debugParticles ? "Debug Particles" : "",
                x,
                (y += h),
            );
            overlayContext.fillText(
                debugRaycast ? "Debug Raycasts" : "",
                x,
                (y += h),
            );
            overlayContext.fillText(
                debugGamepads ? "Debug Gamepads" : "",
                x,
                (y += h),
            );
        }
 ...
 *  @returns {number}
 *  @memberof Utilities */
function distanceAngle(angleA, angleB) {
 ...
 *  @returns {number}
 *  @memberof Utilities */
function lerpAngle(percent, angleA, angleB) {
...
function isOverlapping(posA, sizeA, posB, sizeB = vec2()) {
    return (
        abs(posA.x - posB.x) * 2 < sizeA.x + sizeB.x &&
...
function isIntersecting(start, end, pos, size) {
    ...
    const boxMin = pos.subtract(size.scale(0.5));
 ...
 *  @return {number}
 *  @memberof Random */
function randInt(valueA, valueB = 0) {
...
function randColor(
    ...
    return linear
        ...
        : new Color(
            rand(colorA.r, colorB.r),
            rand(colorA.g, colorB.g),
            rand(colorA.b, colorB.b),
            rand(colorA.a, colorB.a),
        );
}
...
class Color {
    ...
    toString(useAlpha = true) {
        const toHex = (c) =>
            ((c = (clamp(c) * 255) | 0) < 16 ? "0" : "") + c.toString(16);
        return (
            "#" +
            toHex(this.r) +
            toHex(this.g) +
...
class Timer {
     ...
     * @return {number} */
    getPercent() {
        return this.isSet() ? 1 - percent(this.time - time, 0, this.setTime) : 0;
    }
 ...
 *  @param {boolean} pixelated
 *  @memberof Settings */
function setTilesPixelated(pixelated) {
    tilesPixelated = pixelated;
}

/** Set default font used for text rendering
 *  @param {string} font
 *  @memberof Settings */
function setFontDefault(font) {
    fontDefault = font;
}
 ...
 *  @param {boolean} show
 *  @memberof Settings */
function setShowSplashScreen(show) {
 ...
 *  @param {Vector2} size
 *  @memberof Settings */
function setTileSizeDefault(size) {
 ...
 *  @param {number} scale
 *  @memberof Settings */
function setTileFixBleedScale(scale) {
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setEnablePhysicsSolver(enable) {
 ...
 *  @param {number} mass
 *  @memberof Settings */
function setObjectDefaultMass(mass) {
 ...
 *  @memberof Settings */
function setObjectDefaultElasticity(elasticity) {
 ...
 *  @memberof Settings */
function setObjectDefaultFriction(friction) {
 ...
 *  @param {number} speed
 *  @memberof Settings */
function setObjectMaxSpeed(speed) {
 ...
 *  @param {number} scale
 *  @memberof Settings */
function setParticleEmitRateScale(scale) {
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setGamepadsEnable(enable) {
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setGamepadDirectionEmulateStick(enable) {
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setInputWASDEmulateDirection(enable) {
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setTouchInputEnable(enable) {
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setTouchGamepadEnable(enable) {
 ...
 *  @memberof Settings */
function setTouchGamepadAnalog(analog) {
 ...
 *  @param {number} size
 *  @memberof Settings */
function setTouchGamepadSize(size) {
 ...
 *  @param {number} alpha
 *  @memberof Settings */
function setTouchGamepadAlpha(alpha) {
 ...
 *  @param {boolean} enable
 *  @memberof Settings */
function setVibrateEnable(enable) {
 ...
 *  @param {number} range
 *  @memberof Settings */
function setSoundDefaultRange(range) {
 ...
 *  @param {number} taper
 *  @memberof Settings */
function setSoundDefaultTaper(taper) {
 ...
 *  @param {number} time
 *  @memberof Settings */
function setMedalDisplayTime(time) {
 ...
 *  @param {number} time
 *  @memberof Settings */
function setMedalDisplaySlideTime(time) {
 ...
 *  @param {Vector2} size
 *  @memberof Settings */
function setMedalDisplaySize(size) {
 ...
 *  @memberof Settings */
function setMedalsPreventUnlock(preventUnlock) {
 ...
 *  @param {boolean} show
 *  @memberof Debug */
function setShowWatermark(show) {
    showWatermark = show;
}
...
class EngineObject {
    ...
    update() {
        ...
        if (this.mass)
            // don't apply gravity to static objects
        ...
        if (!enablePhysicsSolver || !this.mass)
            // don't do collision for static objects
            return;
        ...
        if (this.groundObject) {
            ...
            const groundSpeed =
                this.groundObject != this && this.groundObject.velocity
        ...
        if (this.collideSolidObjects) {
            ...
            for (const o of engineObjectsCollide) {
                ...
                if (isOverlapping(oldPos, this.size, o.pos, o.size)) {
                    ...
                    debugOverlay &&
                        debugPhysics &&
                        debugOverlap(this.pos, this.size, o.pos, o.size, "#f00");
                    continue;
                }
                ...
                if (smallStepUp || isBlockedY || !isBlockedX) {
                    ...
                    this.pos.y =
                        o.pos.y + (sizeBoth.y / 2 + epsilon) * sign(oldPos.y - o.pos.y);
                ...
                if (!smallStepUp && isBlockedX) {
                    ...
                    this.pos.x =
                        o.pos.x + (sizeBoth.x / 2 + epsilon) * sign(oldPos.x - o.pos.x);
                    if (o.mass) {
                        // inelastic collision
                        const inelastic =
                ...
                debugOverlay &&
                    debugPhysics &&
                    debugOverlap(this.pos, this.size, o.pos, o.size, "#f0f");
            }
        }
            ...
            if (tileCollisionTest(this.pos, this.size, this)) {
                ...
                if (!tileCollisionTest(oldPos, this.size, this)) {
                    ...
                    const isBlockedY = tileCollisionTest(
                        vec2(oldPos.x, this.pos.y),
                        this.size,
                        this,
                    );
                    const isBlockedX = tileCollisionTest(
                        vec2(this.pos.x, oldPos.y),
                        this.size,
                        this,
                    );
                    if (isBlockedY || !isBlockedX) {
                        ...
                        if (wasMovingDown) {
                            ...
                            this.pos.y =
                                ((oldPos.y - this.size.y / 2) | 0) + this.size.y / 2 + epsilon;
                    ...
                    debugOverlay &&
                        debugPhysics &&
                        debugRect(this.pos, this.size, "#f00");
                }
            }
        }
    }
    ...
    setCollision(
        ...
        ASSERT(
            collideSolidObjects || !isSolid,
            "solid objects must be set to collide",
        );
    ...
    renderDebugInfo() {
        if (debug) {
            ...
            const color1 = rgb(
                this.collideTiles ? 1 : 0,
                this.collideSolidObjects ? 1 : 0,
                this.isSolid ? 1 : 0,
                this.parent ? 0.2 : 0.5,
            );
            ...
            this.parent &&
                drawLine(this.pos, this.parent.pos, 0.1, rgb(0, 0, 1, 0.5), false);
        }
    }
}
/**
        ...
        if (textureInfo) {
            ...
            if (tileFixBleedScale) {
                ...
                glDraw(
                    ...
                    color.rgbaInt(),
                    additiveColor && additiveColor.rgbaInt(),
                );
            } else {
                glSetTexture(textureInfo.glTexture);
                glDraw(
...
class FontImage {
    ...
    constructor(
        image,
        tileSize = vec2(8),
        paddingSize = vec2(0, 1),
        context = overlayContext,
    ) {
        // load default font image
        if (!engineFontImage)
            (engineFontImage = new Image()).src =
                "data:image/png;base64,";
    ...
    drawTextScreen(text, pos, scale = 4, center) {
        ...
        (text + "").split("\n").forEach((line, i) => {
            const centerOffset = center
                ? ((line.length * size.x * scale) / 2) | 0
                : 0;
...
function toggleFullscreen() {
    ...
    } else if (rootElement.requestFullscreen) rootElement.requestFullscreen();
}
...
function keyIsDown(key, device = 0) {
    ASSERT(
        device > 0 || typeof key !== "number" || key < 3,
        "use code string for keyboard",
    );
 ...
 *  @memberof Input */
function setInputPreventDefault(preventDefault) {
    inputPreventDefault = preventDefault;
}
...
function gamepadStick(stick, gamepad = 0) {
    return gamepadStickData[gamepad]
        ? gamepadStickData[gamepad][stick] || vec2()
        : vec2();
}
...
function inputUpdatePost() {
    ...
    for (const deviceInputData of inputData)
        for (const i in deviceInputData) deviceInputData[i] &= 1;
    mouseWheel = 0;
}
...
function inputInit() {
    ...
    // init touch input
    if (isTouchDevice && touchInputEnable) touchInputInit();
}
...
function gamepadsUpdate() {
    const applyDeadZones = (v) => {
        const min = 0.3,
            max = 0.8;
        const deadZone = (v) =>
            v > min ? percent(v, min, max) : v < -min ? -percent(-v, min, max) : 0;
    ...
    for (let i = gamepads.length; i--;) {
        ...
        if (gamepad) {
            ...
            for (let j = 0; j < gamepad.axes.length - 1; j += 2)
                sticks[j >> 1] = applyDeadZones(
                    vec2(gamepad.axes[j], gamepad.axes[j + 1]),
                );
...
function touchInputInit() {
    ...
    function handleTouchGamepad(e) {
        ...
        const stickCenter = vec2(
            touchGamepadSize,
            mainCanvasSize.y - touchGamepadSize,
        );
        const buttonCenter = mainCanvasSize.subtract(
            vec2(touchGamepadSize, touchGamepadSize),
        );
        ...
        for (const touch of e.touches) {
            ...
            if (touchPos.distance(stickCenter) < touchGamepadSize) {
                ...
                touchGamepadStick = touchPos
                    .subtract(stickCenter)
                    .scale(2 / touchGamepadSize)
                    .clampLength();
                ...
                touchPos.distance(startCenter) < touchGamepadSize &&
                !wasTouching
            ) {
...
function touchGamepadRender() {
    ...
    const rightCenter = vec2(
        mainCanvasSize.x - touchGamepadSize,
        mainCanvasSize.y - touchGamepadSize,
    );
    for (let i = 4; i--;) {
...
class Sound {
    ...
    constructor(zzfxSound, range = soundDefaultRange, taper = soundDefaultTaper) {
        ...
        if (zzfxSound) {
            ...
            this.randomness =
                zzfxSound[1] != undefined ? zzfxSound[1] : defaultRandomness;
    ...
    play(pos, volume = 1, pitch = 1, randomnessScale = 1, loop = false) {
        ...
        // play the sound
        const playbackRate =
            pitch + pitch * this.randomness * randomnessScale * rand(-1, 1);
        this.gainNode = audioContext.createGain();
        this.source = playSamples(
            this.sampleChannels,
            volume,
            playbackRate,
            pan,
    ...
    getDuration() {
        return (
            this.sampleChannels && this.sampleChannels[0].length / this.sampleRate
        );
    }
...
function playAudioFile(filename, volume = 1, loop = false) {
    ...
    return new SoundWave(filename, 0, 0, 0, (s) =>
        s.play(undefined, volume, 1, 1, loop),
    );
}
        ...
        sequence.forEach((patternIndex, sequenceIndex) => {
            ...
            for (
                ...
                for (
                    ...
                    j++ > beatLength - 99 && stop && attenuation < 1
                        ? (attenuation += 1 / 99)
                        : 0
                ) {
...
function getTileCollisionData(pos) {
    return pos.arrayCheck(tileCollisionSize)
...
class TileLayer extends EngineObject {
    ...
    render() {
        ASSERT(
            ...
            "must call redrawEnd() after drawing tiles",
        );
    ...
    redraw() {
        ...
        for (let x = this.size.x; x--;)
            for (let y = this.size.y; y--;) this.drawTileData(vec2(x, y), false);
        this.redrawEnd();
    }
    ...
    redrawEnd() {
        ASSERT(
            mainContext == this.context,
            "must call redrawStart() before drawing tiles",
        );
        glCopyToContext(mainContext, true);
        //debugSaveCanvas(this.canvas);

        // set stuff back to normal
        [mainCanvas, mainContext, mainCanvasSize, cameraPos, cameraScale] =
            this.savedRenderSettings;
    }
    ...
    drawTileData(layerPos, clear = true) {
        ...
        if (d.tile != undefined) {
            ...
            const pos = layerPos.add(vec2(0.5));
            const tileInfo = tile(
                d.tile,
                s,
                this.tileInfo.textureIndex,
                this.tileInfo.padding,
            );
            drawTile(
                pos,
                vec2(1),
    ...
    emitParticle() {
        ...
        let pos =
            typeof this.emitSize === "number" // check if number was used
                ...
                : vec2(rand(-0.5, 0.5), rand(-0.5, 0.5)) // box emitter
        ...
        const particle = new Particle(
            ...
            this.additive,
            this.trailScale,
            this.localSpace && this,
            this.particleDestroyCallback,
        );
...
class Particle extends EngineObject {
    ...
    render() {
        ...
        const p =
            this.lifeTime > 0 ? min((time - this.spawnTime) / this.lifeTime, 1) : 1;
        ...
        if (this.localSpaceEmitter) {
            ...
            pos = this.localSpaceEmitter.pos.add(
                pos.rotate(-this.localSpaceEmitter.angle),
            );
        ...
        if (this.trailScale) {
            ...
            if (speed) {
                ...
                drawTile(
                    pos.add(direction.multiply(vec2(0, -trailLength / 2))),
                    size,
                    this.tileInfo,
                    color,
                    angle,
...
// Engine internal variables not exposed to documentation
let medalsDisplayQueue = [],
    medalsSaveName,
    medalsDisplayTimeLast;
...
function medalsInit(saveName) {
    ...
    if (!debugMedals)
        medalsForEach(
            (medal) => (medal.unlocked = !!localStorage[medal.storageKey()]),
        );
    ...
    function medalsRender() {
        ...
        if (!medalsDisplayTimeLast) medalsDisplayTimeLast = timeReal;
            ...
            const hidePercent =
                time < medalDisplaySlideTime
                    ? 1 - time / medalDisplaySlideTime
                    : time > slideOffTime
                        ? (time - slideOffTime) / medalDisplaySlideTime
                        : 0;
...
class Medal {
    ...
    render(hidePercent = 0) {
        ...
        // draw the icon
        const gap = vec2(0.1, 0.05).scale(height);
        const medalDisplayIconSize = height - 2 * gap.x;
        this.renderIcon(
            vec2(x + gap.x + medalDisplayIconSize / 2, y + height / 2),
            medalDisplayIconSize,
        );

        // draw the name
        const nameSize = height * 0.5;
        const descriptionSize = height * 0.3;
        const pos = vec2(
            x + medalDisplayIconSize + 2 * gap.x,
            y + gap.y * 2 + nameSize / 2,
        );
    ...
    renderIcon(pos, size) {
        ...
        else drawTextScreen(this.icon, pos, size * 0.7, new Color(0, 0, 0));
    }
...
function glPreRender() {
    ...
    glContext.uniformMatrix4fv(
        ...
        false,
        [s.x, 0, 0, 0, 0, s.y, 0, 0, 1, 1, 1, 1, p.x, p.y, 0, 0],
    );
}
...
function glCreateProgram(vsSource, fsSource) {
    ...
    glContext.attachShader(
        program,
        glCompileShader(vsSource, glContext.VERTEX_SHADER),
    );
    glContext.attachShader(
        program,
        glCompileShader(fsSource, glContext.FRAGMENT_SHADER),
    );
...
function glCreateTexture(image) {
    ...
    if (image && image.width) glSetTextureData(texture, image);
    else {
...
function engineObjectsCollect(pos, size, objects = engineObjects) {
    ...
    if (!pos) {
        // all objects
        for (const o of objects) collectedObjects.push(o);
...
function engineObjectsRaycast(start, end, objects = engineObjects) {
    ...
    debugRaycast &&
        debugLine(start, end, hitObjects.length ? "#f00" : "#00f", 0.02);
    return hitObjects;
}
...
function drawEngineSplashScreen(t) {
    ...
    for (let i = 5; i--;) {
        ...
        circle(60 - i * 6, 30, 10.0, -0.5, PI + 0.5, color(i + 2, 2));
